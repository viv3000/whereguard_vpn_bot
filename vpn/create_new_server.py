...
def gen_wg_ip(i, config):
    return config["ip_local"] + str(i+1) + "/32"


def create_peer(i, key, config):
    return (
        "[Peer]\n" +
        "#" + str(i) + "\n" +
        "PublicKey = " + key + "\n" +
        "AllowedIPs = " + gen_wg_ip(i, config) + "\n\n"
    )


def create_new_server(config, number_of_slots=100):
    keys = create_keys()
    server_id = Data.create_server(config["ip"], config["ipwg"], config["port"], config["interface"], config["wg_interface"], keys[0], keys[1], config["ip_local"])
    server = Data.get_server(server_id)
    Jmax = math.floor(random.random()*1278)+2
    file = (
        "[Interface]\n" +
        "PrivateKey = " + keys[0] + "\n" +
        "Address = " + config["ip_local"] + str(number_of_slots+1) + "/24\n" +
    ...
    for i in range(number_of_slots):
        private_key = gen_private_key()
        Data.create_peer(i, gen_wg_ip(i, config), server_id, private_key, False)
        file += create_peer(i, gen_public_key(private_key), config)
